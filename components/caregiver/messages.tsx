    ...
    <div className="flex-1 overflow-y-auto px-6 py-4">
      <div className="space-y-4">
        {[1, 2, 3, 4, 5].map((i) => (
          ...
          >
            <div
              className={`max-w-[75%] rounded-2xl px-4 py-3 ${i % 2 === 0 ? "bg-sahay-sage/20" : "bg-card border-2 border-border"}`}
            >
              <Skeleton
                className={`h-4 ${i % 3 === 0 ? "w-48" : "w-32"} mb-2`}
              />
...
export function Messages({ onClose }: MessagesProps) {
  const { data, isDataLoading, sendMessage, markMessageRead, getUnreadCount } =
    useSahay();
  ...
  useEffect(() => {
    const unread = messages.filter(
      (m) => !m.isRead && m.from === "careReceiver",
    );
    ...
    <div className="h-full flex flex-col bg-background">
              ...
              {messages.map((message, idx) => {
                ...
                const showDateSeparator =
                  ...
                  getDateLabel(message.timestamp) !==
                    getDateLabel(prevMessage.timestamp);
                  ...
                  <div key={message.id}>
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={`flex items-center gap-1 mt-1.5 ${isFromMe ? "justify-end" : ""}`}
                        >
                          <p
                            className={`text-[11px] ${isFromMe ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                          >
          ...
          {messages.length === 0 && (
            ...
            >
              <div className="w-20 h-20 rounded-full bg-sahay-sage-light/50 flex items-center justify-center mx-auto mb-5">
                <MessageCircle
                  className="w-10 h-10 text-sahay-sage/50"
                  strokeWidth={1.5}
                />
              </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          >
            <Send
              className={`w-5 h-5 text-primary-foreground transition-transform ${newMessage.trim() ? "translate-x-0" : ""}`}
            />
          </motion.button>
        </div>
      </motion.div>
